...
DoneScreen::DoneScreen(QWidget *parent) : QWidget(parent) { buildUi(); }
...
void DoneScreen::buildUi() {
    ...
    connect(return_starting_screen, &QPushButton::clicked, this, [this] { emit StartScreenSwitch(); });
    ...
    connect(return_charts_screen, &QPushButton::clicked, this, [this] { emit BacktestScreenSwitch(); });
...
void DoneScreen::setResults(const QString &final_stats) {
    if (final_result) {
        final_result->setPlainText(final_stats);
    }
}

QString DoneScreen::resultsText() const {
    return final_result ? final_result->toPlainText() : QString{};
}

QPushButton *DoneScreen::homeButton() const {
    return return_starting_screen;
}

QPushButton *DoneScreen::chartsButton() const {
    return return_charts_screen;
}
